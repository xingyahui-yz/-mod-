...
> 状态：v1.90 — 由项目所有者与协作 agent 通过开发迭代同步
...
- **建议 Card ID（Proposed Card ID）**：AI 为新 Card 提案提供的 PascalCase 候选标识符；待处理期间不占用 ID。接受确认允许用户编辑最终 ID 并即时校验，冲突时提案保持待处理且禁止接受，不自动追加数字或改名；只有接受成功后才成为不可变 Card ID 并占用命名空间。
  _避免：待处理提案预留 ID、冲突时静默追加后缀或改名_
...
- **项目级 AI 对话（Project AI Conversation）**：每个 Mod 项目唯一的一段活动 AI 多轮对话，可在明确展示目标与变化后创建或修改该项目中的多张 Card；不属于某一张 Card，打开项目时恢复。项目可另有只读对话归档，但不能同时存在多段活动对话。新项目在首次发送用户消息时才正式建立活动对话；仅打开项目或展开抽屉不会创建空历史。运行中的 AI 轮次属于当前项目；切换项目必须先完成或由用户确认取消并记录该轮次，不能让旧项目请求在后台完成或写入新项目。
- **项目 AI 对话抽屉（Project AI Conversation Drawer）**：项目界面中持续挂载、可折叠的右侧面板，是项目级 AI 对话、轮次状态与逐 Card 提案确认的主入口；切换 Card 或项目内功能页不销毁对话状态，点击提案可将主编辑区定位到目标 Card。v0.10 不再把独立 AI 页签作为主入口。
  _避免：把对话历史挂在当前选中 Card、一个项目维护多段并行活动对话、允许模型静默切换项目_
- **对话项目基线（Conversation Project Baseline）**：每轮 AI 请求开始时读取的当前项目源数据；已接受提案和用户手工编辑属于基线，待处理、已拒绝或过期提案不属于基线。
  _避免：把 AI 候选状态当成已发生事实、维护与项目源数据分叉的影子项目_
- **对话 Card 上下文（Conversation Card Context）**：每轮提供给模型的全项目 Card 目录摘要与用户显式附加的 Card 完整文档；附件可通过 Card ID 选择并显示为上下文项，当前选中 Card 只作为建议而不默认发送。目录默认包含 Card 身份、基础属性、截短描述、行为 kind 摘要与 revision；超过预算时可见地降级为只含 ID、名称、类型和 revision 的紧凑目录。显式附件优先且不可截断；连紧凑目录都无法容纳时阻止发送。每轮永久记录实际附加的 Card ID 与 revision；未附加时不得把“这张卡”猜成当前选择。
  _避免：每轮发送全部 Card 全文、暗中发送当前 Card、静默截断目录、缺少完整文档时猜测修改_
- **Card 上下文补取（Card Context Expansion）**：模型仅凭目录摘要无法安全完成一轮时，请求额外 Card 完整文档的内部步骤；每个用户轮次最多补取一次，补取后的调用必须结束该轮。
  _避免：无限递归补取、把内部补取显示成新的用户轮次_
...
- **AI 提案组（AI Proposal Set）**：项目级 AI 对话中一轮产生的零到多个 Card 级 AI 提案；每张 Card 独立接受、拒绝或过期，不形成跨 Card 原子事务。
- **Card 提案预览（Card Proposal Preview）**：从对话抽屉打开、不会写入项目的只读比较视图。修改提案会将主编辑区定位到目标 Card，并比较当前内容与候选内容；新建提案只显示候选 Card，不创建目录项或占用建议 ID。只有接受操作才提交修改或创建，离开预览不改变提案状态。
- **原子 Card 提案（Atomic Card Proposal）**：一张 Card 提案的差异可按基础属性、触发器、节点和连线分区展示，但不能部分接受；用户只能接受或拒绝整张 Card。接受时 Card 与行为图作为一个编辑事务写入，并形成一个撤销步骤。
  修改现有 Card 时，用户已在差异预览中完成审阅，点击“接受整张 Card（可撤销）”即可直接提交，不再二次确认；新建 Card 仍需确认最终 ID。
  v0.10 的可执行提案只创建或修改 Card；删除只能作为文字建议，由用户通过 Card 回收站流程执行。提案不得引用尚未接受的新 Card，目标 Card 落盘后必须重新生成依赖提案。
  _避免：整轮提案必须全收或全拒、因一张 Card 过期阻止其他 Card、让 AI 直接删除 Card、自动重写跨提案引用_
- **零提案轮次（Proposal-free Turn）**：AI 只进行分析、解释、比较或澄清而不产生 Card 提案的一轮项目级对话；不会为了满足输出格式而制造修改。
  _避免：强制每轮至少修改一张 Card_
- **快速回复选项（Quick Reply Option）**：AI 可随回复提供的零到四个建议回答；点击只把选项文字和选项 ID 带入用户输入草稿，用户可编辑后再发送，不会立即调用模型或执行任何 Card 操作。自由输入会取消选项关联，文字输入框始终可用。
  _避免：点击即产生 API 消耗、把快速回复当作提案接受或项目命令、只允许按钮而禁止自由回答_
- **AI 轮次尝试（AI Turn Attempt）**：同一用户消息的一次模型执行；可完成、失败、被用户取消或因应用退出而中断。重试保持原用户消息，在同一轮次追加读取最新项目基线的新尝试；修改原文后发送则创建新用户轮次。失败、取消和中断尝试保留在完整历史中，但不进入语义摘要或改变提案状态；中断后只允许手动重试。只有经校验的完整助手响应与对话文档原子保存成功后，该尝试才完成且回复才对用户可见；最终保存失败不得展示或稍后补写旧响应。
  _避免：失败后删除用户消息、把重试伪装成首次执行、在历史中保存敏感认证信息、显示无法恢复的幽灵回复、用过期响应延迟覆盖新基线_
- **过期 AI 提案（Stale AI Proposal）**：请求期间目标 Card revision 已变化的修改提案；仍可查看但不得应用，提案组中的其他 Card 不受影响。新建提案不因无关编辑过期，接受时另做 ID 冲突检查。
  “基于当前版本重做”会创建一条可编辑的新用户消息并开始新轮次，读取最新项目基线；它不是原尝试重试，旧提案永久保持过期状态。
  _避免：用旧提案覆盖请求之后的用户编辑、因一张 Card 变化使整组提案过期_
- **被取代 AI 提案（Superseded AI Proposal）**：用户继续对话并要求修改某个待处理候选后，被该 Card 的新提案替代的旧提案；保留在对话记录中但不再允许应用。
  _避免：同一 Card 同时存在多个可应用候选、把被取代提案重新当成待处理提案_
- **已拒绝 AI 提案（Rejected AI Proposal）**：用户明确拒绝的 Card 提案；内容继续保留在完整对话历史中，但状态不可恢复，也不能随后接受。改变主意必须基于当前项目重新生成。
  _避免：把拒绝当成暂存、重新启用旧 revision 提案_
- **提案拒绝确认（Proposal Rejection Confirmation）**：待处理提案只有经过二次确认才可进入 `rejected`；确认界面必须明确告知拒绝不可恢复、改变主意需要重新生成。取消确认保持原提案状态不变。
  _避免：误触后永久失去待处理提案、用可撤销提示掩盖不可恢复语义_
- **提案拒绝反馈（Proposal Rejection Feedback）**：用户确认拒绝时可选填写的结构化原因；未填写不阻止拒绝，填写后随拒绝事件永久保存并进入后续 AI 对话上下文，帮助避免重复不合意方案。
  _避免：强迫用户解释拒绝、丢失已经明确表达的设计偏好_
- **已撤销 AI 提案（Reverted AI Proposal）**：曾被用户接受、随后对应 Card 编辑事务被撤销的提案；保留接受与撤销事实，不能直接再次接受，只有重做同一事务后才恢复为已接受。
  _避免：撤销后恢复成待处理、抹去曾经接受的历史_
- **对话历史（Conversation History）**：保留多轮对话的上下文
- **对话文档（Conversation Document）**：一个 Mod 项目唯一项目级 AI 对话的版本化权威记录，包含完整的产品可见消息、经 schema 校验的结构化提案、轮次尝试、提案状态和可重建摘要缓存，以及 provider、model、时间、token 用量、请求 ID 等必要诊断元数据。失败仅保存脱敏的错误分类与简短信息；不保存 provider 原始响应、隐藏推理或认证信息。
  _避免：按 Card 拆分对话历史、只保存当前上下文窗口、在文档中保存 API Key、永久耦合供应商原始协议或泄漏隐藏推理_
- **对话隔离（Conversation Quarantine）**：活动对话文档损坏或属于未知未来 schema 时的保真只读状态；原文件不被覆盖，Card 编辑保持可用，但在用户导出或保留原始数据并明确创建新对话前禁止继续发送。已知旧 schema 先备份，再逐版本迁移。
  _避免：打开项目即重置坏对话、因 AI 历史损坏阻断 Card 编辑、在未知 schema 上继续写入_
- **对话容量阈值（Conversation Capacity Thresholds）**：活动 `conversation.json` 超过 10 MB 或 5,000 条消息时进入软警告；超过 50 MB 或 20,000 条消息时进入硬限制。越过硬限制时允许当前已发送轮次完成并持久化，但禁止开始下一轮，直到成功归档并重置活动对话。
  _避免：无提示无限增长、在模型执行中途因容量取消、超过硬限制后继续积累_
- **SQLite 升级触发条件（SQLite Upgrade Triggers）**：v0.10 不自动切换存储；当任一真实条件出现时正式立项迁移：软阈值规模下活动对话加载或原子保存 p95 超过 500 ms；需要跨归档全文搜索、筛选或统计；需要局部更新或并发写入；真实项目频繁触达硬上限且归档重置妨碍工作。
  _避免：因预想规模过早引入数据库、在 JSON 已出现可测瓶颈后仍无限延期迁移_
- **对话归档（Conversation Archive）**：达到历史硬上限后，在项目内原子保存并校验的完整只读对话记录；不进入后续模型上下文，归档成功后才允许重置活动对话，并可额外导出副本。
  用户也可在未达到上限时主动执行“归档并开始新对话”；操作必须二次确认，项目内归档写入与读回校验成功后才重置活动对话，旧对话中的待处理提案随归档失效且不能在新对话中恢复。
  _避免：未归档直接清空、只依赖外部导出、同时恢复为第二段活动对话_
- **对话滚动摘要（Conversation Rolling Summary）**：从完整对话历史派生的早期上下文摘要，与最近消息、当前项目基线和相关待处理提案共同提供给模型；摘要可重建，不替代或删除原始记录。最近消息按 token 预算从近到远加入且保持每条完整，始终保留最新用户消息，并为模型输出与最多一次 Card 上下文补取预留空间；若最新消息本身超过可用预算，发送前阻止并提示缩短，不静默截断。
  _避免：只保存摘要、因压缩删除原始消息、把摘要当成项目事实、固定轮数在长消息下溢出上下文、截断用户意图_
- **混合对话摘要（Hybrid Conversation Summary）**：对话滚动摘要中，提案状态、Card ID 与轮次由本地确定性派生，用户目标和偏好由 LLM 压缩。只有已完成轮次使未摘要消息超过 token 阈值时才后台刷新 LLM 摘要；失败、取消或中断尝试不触发。刷新成功后旧消息才进入摘要覆盖区；失败则保留旧摘要与未摘要消息并继续对话。摘要调用不产生可见轮次或提案，也不能改变项目事实。
  _避免：让 LLM 判断提案是否已接受、用摘要回写项目源数据、每轮额外付费摘要、摘要失败丢失上下文_
...
└── .modstudio/                 ← 工具私有元数据（用户不直接编辑）
    ...
    ├── ai/
    │   ├── conversation.json   ← 当前项目唯一的活动 AI 对话
    │   └── archives/           ← 只读归档（<conversation-id>.json）
...
**v0.10 当前主线**
- 🛠️ Card AI 对话式迭代（多轮）：对话骨架与逐 Card 提案已实现；第二切片补强 create transaction receipt、单实例 Card ID claim 回收、Card 删除联合终检及 uncertain autosave 恢复。当前工作树第一遍为 52 个测试文件/648 项通过、renderer/node TypeScript 与 diff-check 通过；仍待构建产物检查和新固定提交的独立复检
...
- **Card**：v0.9 端到端闭环 ✅ → CardCatalog 深模块 ✅ → v0.10 AI 多轮对话
- **v0.10 实施**：四个短生命周期 `codex/` 分支逐片合并；第二切片结束旧 AI 双入口，第四切片通过 release gate 后完成
- **实体扩展**：Relic 与其余实体暂后于 Card AI 多轮对话
...
| 2026-08-27 | v1.35 同步 v0.9 完成状态与 CardCatalog 重构，并锁定 v0.10 优先完成 Card AI 多轮对话 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.36 定义项目级 AI 对话 — 对话绑定 Mod 项目，可在同一对话中创建或修改多张 Card | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.37 定义 AI 提案组 — 一轮可产生多个 Card 提案，每张独立确认、拒绝和处理过期 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.38 定义对话项目基线 — 下一轮始终以当前项目源数据为事实，AI 候选状态不形成影子项目 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.39 定义被取代 AI 提案 — 可带待处理提案继续对话，新候选只替换被继续修改的同 Card 旧候选 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.40 定义建议 Card ID — 新 Card 提案只提供候选 ID，接受时由用户确认、检查冲突并正式占用 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.41 收紧项目级 AI 对话 — 每个 Mod 项目只有一段持久化对话，打开项目时恢复 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.42 定义对话滚动摘要 — 完整历史永久保存，模型请求使用可重建摘要与最近消息控制上下文 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.43 定义混合对话摘要 — 系统事实本地派生，用户目标与偏好由 LLM 压缩，摘要失败不阻塞对话 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.44 锁定 v0.10 AI 操作范围 — 可执行提案仅创建或修改 Card，删除保留为文字建议与用户操作 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.45 定义零提案轮次 — AI 可只回复分析或澄清，提案组允许为空 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.46 定义对话 Card 上下文 — 发送全项目目录摘要，只展开相关 Card 完整文档 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.47 定义 Card 上下文补取 — 每轮最多一次内部完整文档补取，第二次调用必须结束该轮 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.48 扩展过期 AI 提案 — 请求期间允许继续编辑，修改提案按目标 Card revision 独立判定过期 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.49 定义 AI 轮次尝试 — 取消和失败保留并可重试，重试读取最新项目基线且不污染摘要或提案状态 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.50 扩展 AI 轮次尝试 — 发送前持久化运行状态，异常退出后标记中断并只允许手动重试 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.51 定义对话文档 — v0.10 以每项目单一版本化 JSON 保存完整对话，并为未来 SQLite 迁移保留出口 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.52 定义对话归档 — 每项目仅一段活动对话；达到硬上限后归档完整历史并重置，归档不进入模型上下文 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.53 收紧对话归档 — 项目内原子保存并校验后才重置活动对话，同时允许额外导出副本 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.54 限制 AI 提案依赖 — 提案不得引用尚未接受的新 Card，真实 ID 落盘后需重新生成依赖提案 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.55 定义已拒绝 AI 提案 — 拒绝为不可恢复终态，改变主意必须基于当前项目重新生成 | 用户 + Codex `/grill-with-docs` |
| 2026-08-27 | v1.56 定义已撤销 AI 提案 — 接受后撤销记为 reverted，重做同一 Card 事务后恢复 accepted | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.57 定义项目 AI 对话抽屉 — 以持续挂载、可折叠的右侧面板承载对话和逐 Card 提案确认，替代独立 AI 页签主入口 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.58 定义 Card 提案预览 — 修改提案定位目标 Card 并显示当前值与候选差异，新建提案只读预览且不占 ID，只有接受才写入项目 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.59 定义原子 Card 提案 — 差异按领域区域展示，但一张 Card 只能整体接受或拒绝，接受形成单个编辑历史步骤 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.60 定义提案拒绝确认 — 拒绝前必须二次确认并提示不可恢复，取消确认不改变提案状态 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.61 定义提案拒绝反馈 — 拒绝原因可选，填写后结构化保存并提供给后续 AI，空原因不阻止拒绝 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.62 收紧建议 Card ID — 接受时可编辑并即时校验最终 ID，冲突保持 pending 且不自动改名，成功后才占用 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.63 明确提案接受确认 — 现有 Card 在差异预览后直接整体接受并可撤销，不再弹窗；新 Card 仍确认最终 ID | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.64 扩展主动对话归档 — 未达上限也可二次确认后归档并新建活动对话，旧待处理提案随归档永久失效 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.65 收紧对话持久化边界 — 保存校验后的产品语义及诊断元数据，不保存 provider 原始响应、隐藏推理或未脱敏错误 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.66 定义对话 token 预算 — 最近消息按预算完整纳入并预留输出与补取空间，最新消息单独超限则阻止而不截断 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.67 定义摘要刷新 — 已完成轮次越过 token 阈值才后台刷新，失败保留旧摘要与未摘要消息且不阻塞对话 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.68 定义过期提案重做 — 基于当前版本重做属于读取最新项目基线的新用户轮次，旧提案永久保持 stale | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.69 定义项目级对话路径 — 活动对话固定存于 `.modstudio/ai/conversation.json`，只读归档按 conversation ID 存放 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.70 补全对话容量阈值 — 10 MB/5,000 消息软警告，50 MB/20,000 消息硬限制，当前轮完成后禁止继续 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.71 定义 SQLite 升级触发条件 — 以可测性能、跨归档查询、局部并发写入或频繁硬上限为迁移立项依据 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.72 接受 ADR-0007 — 固化项目级 Card AI 对话、逐 Card 提案生命周期、上下文压缩与 JSON 持久化边界 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.73 定义快速回复选项 — AI 可提供零到四个可编辑回答草稿，点击不自动发送且不得执行 Card 操作 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.74 定义对话隔离 — 损坏或未知 schema 的对话保真只读且不影响 Card 编辑，明确保留原文件后才可新建对话 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.75 收紧对话项目归属 — AI 运行中切换项目必须先完成或确认取消并持久化，禁止旧请求后台完成或跨项目写入 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.76 定义回复可见提交点 — 完整响应只有原子保存成功后才可见，保存失败不展示且不得延迟补写旧响应 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.77 定义对话建立时点 — 首次发送才创建活动对话文档，仅打开项目或展开抽屉不写入空历史 | 用户 + Codex `/grill-with-docs` |
| 2026-08-28 | v1.78 区分重试与修改后发送 — 重试在原轮次追加 attempt，修改文本则创建读取最新项目基线的新用户轮次 | 用户 + Codex `/grill-with-docs` |
