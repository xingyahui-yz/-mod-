...
- **项目级 AI 对话（Project AI Conversation）**：每个 Mod 项目唯一的一段活动 AI 多轮对话，可在明确展示目标与变化后创建或修改该项目中的多张 Card；不属于某一张 Card，打开项目时恢复。项目可另有只读对话归档，但不能同时存在多段活动对话。
...
- **对话 Card 上下文（Conversation Card Context）**：每轮提供给模型的全项目 Card 目录摘要与相关 Card 完整文档；相关 Card 包括用户明确提及、当前选中、存在待处理提案或近期对话直接涉及的 Card。
  _避免：每轮发送全部 Card 全文、缺少完整文档时猜测修改_
...
- **AI 轮次尝试（AI Turn Attempt）**：同一用户消息的一次模型执行；可完成、失败、被用户取消或因应用退出而中断，重试会基于最新项目基线产生新的尝试。失败、取消和中断尝试保留在完整历史中，但不进入语义摘要或改变提案状态；中断后只允许手动重试。
  _避免：失败后删除用户消息、把重试伪装成首次执行、在历史中保存敏感认证信息_
...
## 6. 当前状态（截至 2026-08-27，v0.9 Card 端到端已完成）
...
**架构与决策**
- ✅ 6 份 ADR（0001-0006），v0.9 Card 领域语义、数据安全和交付门槛已锁定并实现
...
**v0.10 当前主线**
- ❌ Card AI 对话式迭代（多轮）：在现有完整 AI 提案、revision 过期保护与统一编辑历史之上扩展
