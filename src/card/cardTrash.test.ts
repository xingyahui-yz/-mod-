...
describe('Card trash repository', () => {
  it('将 CardDocument 与现有 C# 一起移入回收站', async () => {
    ...
    expect(files.files.get(`${project}/.modstudio/trash/Fireball-1/card.json`)).toBe(document)
    expect(files.files.get(`${project}/.modstudio/trash/Fireball-1/artifact.cs`)).toBe('generated')
  })
  ...
  it('暂存失败时补偿恢复 Card，活动文件保持完整', async () => {
    ...
    files.failRenameTo = `${project}/.modstudio/trash/Fireball-1/artifact.cs`
  ...
  it('恢复前按忽略大小写检查 ID 冲突，并支持无 C# Card 恢复', async () => {
    const files = new MemoryFiles()
    files.files.set(`${project}/.modstudio/trash/Fireball-1/card.json`, document)
    files.directories.add(`${project}/.modstudio/trash`)
    files.directories.add(`${project}/.modstudio/trash/Fireball-1`)
    files.files.set(`${project}/.modstudio/cards/fireball.json`, document)
  ...
  it('恢复产物失败时 CardDocument 补偿回回收站', async () => {
    const files = new MemoryFiles()
    files.files.set(`${project}/.modstudio/trash/Fireball-1/card.json`, document)
    files.files.set(`${project}/.modstudio/trash/Fireball-1/artifact.cs`, 'generated')
    files.directories.add(`${project}/.modstudio/trash`)
    files.directories.add(`${project}/.modstudio/trash/Fireball-1`)
